...
- 5.0 ≤ score < 7.5 → "consider"
- score < 5.0 → "rejected"

Важно: "rejected" присваивай ТОЛЬКО при ПОДТВЕРЖДЁННЫХ слабых ответах или явных red flags в транскрипте. Нейтральные / неполные данные → "consider", а НЕ "rejected". Отсутствие данных не основание для отказа.
...
Без транскрипта ставь scores4 консервативно (6/10 по каждой шкале по умолчанию — нейтральный сигнал), confidencePercent в диапазоне 15..40, decision = "consider". "rejected" без транскрипта НЕ ставь — отсутствие данных не подтверждает слабость кандидата. Исключение: если в JD есть формальный hard-requirement (например "обязательно английский C1"), которому кандидат ЯВНО не соответствует по данным карточки — тогда допустимо "rejected" с risks-блоком, объясняющим hard-блокер.`;
